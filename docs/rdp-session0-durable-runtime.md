...
foreach ($port in 80,4900,5900,5910,4010,5010,4080,5080,4091,5091,4310,5310,4320,5320,5370,4370,5432) {
...
>
> **Coverage note (2026-08-06):** **production-house PREPROD `:4370`** (`production-house-staging.delena.buzz`) had the identical gap, fixed the same way — `StackPilot-S0-ProductionHouse-Preprod` (AtStartup) via `register-s0-app-tasks.ps1` (`F:\apps\production-house\start.ps1`, same bespoke `vite preview` launcher pattern) plus a matching `Write-ViteLauncher` entry in `start-fleet-session0.ps1`. Verify `:4370` too whenever confirming fleet durability — both `:5370` and `:4370` are now covered.
